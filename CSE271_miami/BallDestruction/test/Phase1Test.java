...
public class Phase1Test {
     ...
     */
    private static final Rectangle PanelBounds =
            new Rectangle(0, 0, WIDTH, HEIGHT);

    /**
     * Internal helper method to determine number of steps enemy has to
     * move before its direction changes.
     * 
    ...
    public void testGamePlay() {
        ...
        for (int i = 0; (i < 6); i++) {
            be.move(WIDTH,  HEIGHT);
            //System.out.format("BigEnemy: %d, %d, %d, %d\n", be.getX(), be.getY(), be.getX() + be.getWidth(), be.getY() + be.getHeight());
            se.move(WIDTH,  HEIGHT);
            //System.out.format("SmallEnemy: %d, %d, %d, %d\n", se.getX(), se.getY(), se.getX() + se.getWidth(), se.getY() + se.getHeight());
            for (int j = 0; (j < missileList.size()); j++) {
                missileList.get(j).move(WIDTH, HEIGHT, missileList, j);
                //System.out.format("Missile: %d, %d, %d, %d\n", missileList.get(j).getX(), missileList.get(j).getY(), missileList.get(j).getX() + missileList.get(j).getWidth(), missileList.get(j).getY() + missileList.get(j).getHeight());
            }
