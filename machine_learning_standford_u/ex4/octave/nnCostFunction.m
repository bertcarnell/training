...
# 1 x 1 - note: sum(partial) does the column sums
J = 1/m*sum(sum(partial));
...
J = J + lambda / 2 / m * (sum(sum(temp_theta1.*temp_theta1)) + sum(sum(temp_theta2.*temp_theta2)));
...
Theta1_grad = (1 / m) .* (d2' * X) + (lambda / m) .* temp_theta1;

# k x s+1 = (m x k)' * (m x s+1) + (k x s+1)
Theta2_grad = (1 / m) .* (d3' * a2) + (lambda / m) .* temp_theta2;
