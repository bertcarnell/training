...
function ret = loss(model, data, wd_coefficient)
  ...
  % size(log_class_prob) c x N
  % size(data.targets) c x N
  % size(sum(log_class_prob .* data.targets, 1)) 1 x N
  % size(mean(sum(log_class_prob .* data.targets, 1))) 1 x 1
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  m = size(model.input_to_hid, 1);
  % N = <number of data cases>
  N = size(data.inputs, 2);
  % c = number of classes
  c = size(model.hid_to_class, 1);
  % p = number of inputs
  p = size(model.input_to_hid, 2);
  wd_loss_gradient = model_to_theta(model) * wd_coefficient; % (m*p + m*c) x 1
  
  % use internals from the loss function
  hid_input = model.input_to_hid * data.inputs; % m x N = m x p * p x N
  hid_output = logistic(hid_input); % m x N
  class_input = model.hid_to_class * hid_output; % c x N = c x m * m x N
  class_normalizer = log_sum_exp_over_rows(class_input); % 1 x N
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]);  % c x N
  class_prob = exp(log_class_prob); % c x N
  
  % m x N = (m x N * m x N)
  temp_partial = hid_output .* (1-hid_output);
  
  % m x p = m x c * (c x N - c x N) .* m x N * N x p
  classification_loss_gradient_1 = transpose(model.hid_to_class) * (class_prob - data.targets) .* temp_partial * transpose(data.inputs) / N;

  % c x m = (c x N - c x N) * N x m
  classification_loss_gradient_2 = (class_prob - data.targets) * transpose(hid_output) / N;
  
  classification_loss_gradient_1_transpose = transpose(classification_loss_gradient_1);
  classification_loss_gradient_2_transpose = transpose(classification_loss_gradient_2);
  classification_loss_gradient = [classification_loss_gradient_1_transpose(:); classification_loss_gradient_2_transpose(:)];
  ret = theta_to_model(wd_loss_gradient + classification_loss_gradient);
end
