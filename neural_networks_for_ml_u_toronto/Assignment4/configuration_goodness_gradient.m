...
%m = size(hidden_state, 1);
%N = size(visible_state, 1);
%d_G_by_rbm_w = zeros(m, N);
%for i = 1:m
%  for j = 1:N
%    d_G_by_rbm_w(i, j) = mean(visible_state(j,:) .* hidden_state(i,:));
%  endfor
%endfor

c = size(hidden_state, 2);
d_G_by_rbm_w = hidden_state * transpose(visible_state) / c;
